...
# Extract API_URL environment variables and build the sed expression
env | grep '^API_URL' | while IFS='=' read -r key value; do
...
if [ ! -s "$sed_script" ]; then
    echo "No API_URL environment variables found. Exiting with error."
    exit 1
fi
